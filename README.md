...
[`id` belongs to `relational-pouch`](https://github.com/pouchdb-community/relational-pouch#how-does-it-work), 
whereas [`_id`](https://docs.couchdb.org/en/latest/best-practices/documents.html) 
belongs to PouchDB/PouchDB. `relational-pouch` uses `id` for references to related objects. 
You _can_ put the same value for `id` as `_id`, but if one is autogenerated you might 
want to autogenerate the other. Do what makes sense for your use case!
