...
exports.setSchema = function (schema) {
  ...
  function _saveAll(type, json) {
    ...
    if (Array.isArray(json)) {
      return Promise.resolve().then(function () {
        pouchDocs = json.map(function (obj, i) {
          obj.id = i + 1;
          return toRawDoc(typeInfo, obj);
        });
        return db.bulkDocs(pouchDocs).then(function (results) {
          return results;
        }).catch(function (err) {
          console.error(err);
        });
      }).then(function (pouchResults) {
        pouchResults.map(function (pouchRes, i) {
          return [extend(true, pouchDocs[i], {
            id: deserialize(pouchRes.id),
            rev: pouchRes.rev
          })];
        });
        var res = {};
        res[typeInfo.plural] = pouchResults;
        return res;
      });
    }
  }
